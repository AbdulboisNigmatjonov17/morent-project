...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="w-full h-[12vh] flex items-center justify-between bg-white px-5">
        <div className="leftSide flex gap-20 items-center max-w-[70%] bg-inherit">
          <Logo />
          <form className='border-[#c1c1c1] bg-inherit border-[1px] rounded-xl text-[20px] gap-32 px-4 py-2 max-w-screen-sm flex justify-between'>
            <div className='flex gap-5 bg-inherit'>
              <label htmlFor="search">
                <SearchTwoToneIcon fontSize='large' className='cursor-pointer bg-white opacity-50 hover:opacity-100' />
              </label>
              <input type="text" id='search' placeholder='Search something here' className=' focus:outline-none bg-white' />
            </div>
            <label htmlFor="search">
              <TuneTwoToneIcon fontSize='large' className='cursor-pointer bg-white opacity-50 hover:opacity-100' />
            </label>
          </form>
        </div>
        <div className='rightSide max-w-[30%] flex justify-start bg-white'>
          <div className=' flex w-full items-center gap-5 bg-inherit'>
            <div className="icon rounded-full border-[#c1c1c1] border-[1px] bg-inherit p-2">
              <SettingsTwoToneIcon className='rounded-full cursor-pointer' />
            </div>
            <div className="icon rounded-full border-[#c1c1c1] bg-inherit border-[1px] p-2">
